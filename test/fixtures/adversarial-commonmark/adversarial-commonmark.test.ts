...
describe("Adversarial CommonMark — D1 grammar verification", () => {
	...
	for (const id of ADVERSARIAL_IDS) {
		describe(id, () => {
			...
			// AC6 unrecognizedCount stays RED until D2 (residual-bracket scanner) ships.
			// Un-skip when extractLinks returns `{ links, unrecognized }`.
			const unrecognizedTest =
				id === "AC6" && expected.unrecognizedCount > 0 ? it.skip : it;
			unrecognizedTest(
				`emits ${expected.unrecognizedCount} unrecognized record(s)`,
				() => {

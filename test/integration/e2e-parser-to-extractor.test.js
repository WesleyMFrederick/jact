    ...
    expect(decisions).toEqual([
      {
        ...
        decision: { eligible: false, reason: 'Full-file link ineligible without --full-files flag' }
      },
      {
        link: '[Full file with force marker](another.md)',
        decision: { eligible: true, reason: 'force-extract overrides defaults' }
      },
      {
        link: '[Section link](#header)',
        decision: { eligible: true, reason: 'Markdown anchor links eligible by default' }
      },
      {
        link: '[Section with stop marker](#section)',
        decision: { eligible: false, reason: 'stop-extract-link marker prevents extraction' }
      }
    ]);
  });

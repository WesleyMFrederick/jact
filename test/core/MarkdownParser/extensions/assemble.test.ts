...
describe("assemble — combined Obsidian-style extension set", () => {
	it("returns one combined syntax extension and four fromMarkdown extensions", () => {
		expect(jactSyntaxExtension()).toBeTypeOf("object");
		expect(jactMdastExtensions()).toHaveLength(4);
	});

	it("parses all four custom syntaxes in a single document", () => {
		const md =
			"A ==highlight==, a %%comment%%, a [cite: docs/spec.md], and ^block-ref-1.";

...
describe("ParsedDocument.hasAnchor — Obsidian Invalid Chars (#1)", () => {
	it("should match heading with colon when anchor has colon stripped", () => {
		...
		const headings = [
			{ level: 3, text: "ADR-006: Title", raw: "### ADR-006: Title\n" },
		];
		const anchors = [
			{
				anchorType: "header",
	...
	it("should match heading with colon using exact id match", () => {
		const headings = [
			{
				level: 3,
				text: "MEDIUM-IMPLEMENTATION: Patterns",
				raw: "### MEDIUM-IMPLEMENTATION: Patterns\n",
			},
		];
		const anchors = [
			{
	...
	it("should still match anchors without colons (no regression)", () => {
		const headings = [
			{ level: 2, text: "Simple Heading", raw: "## Simple Heading\n" },
		];
		const anchors = [
			{
				anchorType: "header",
	...
	it("should match URL-decoded anchor against urlEncodedId after decoding", () => {
		const headings = [
			{ level: 3, text: "Story 1.5: Cache", raw: "### Story 1.5: Cache\n" },
		];
		const anchors = [
			{
				anchorType: "header",

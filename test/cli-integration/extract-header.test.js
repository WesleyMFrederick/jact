...
describe("CLI - extract header command", () => {
  ...
  it("should exit with code 1 when header not found", async () => {
    ...
    let exitCode;
    try {
      runCLI(`node tools/citation-manager/src/citation-manager.js extract header "${targetFile}" "${invalidHeader}"`);
    } catch (error) {
  ...
  it("should display validation error with suggestion", async () => {
    ...
    let stdout;
    try {
      runCLI(`node tools/citation-manager/src/citation-manager.js extract header "${targetFile}" "${similarHeader}"`);
    } catch (error) {
      stdout = error.stdout;
    }
...
describe("CLI Help Documentation", () => {
  it("should show extract header in top-level help", async () => {
    ...
    const output = runCLI("node tools/citation-manager/src/citation-manager.js extract --help");
  ...
  it("should show detailed help for extract header subcommand", async () => {
    ...
    const output = runCLI("node tools/citation-manager/src/citation-manager.js extract header --help");
